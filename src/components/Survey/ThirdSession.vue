...
const respond = await GET(
  "/%E6%9C%83%E5%8B%98%E5%8F%8A%E5%9C%B0%E6%96%B9%E5%BB%BA%E8%A8%AD(%E7%AC%AC%E4%B8%89%E6%9C%83%E6%9C%9F)?maxRecords=50&view=Grid%20view"
);
...
<template>
  <div class="thirdSession">
    ...
    <div class="thirdSession_content" v-for="item in filterData" :key="item.id">
        ...
        <div v-if="item.fields.結論與照片 != undefined" class="case_photo_list">
          ...
          <div class="case_photo_container">
            <div class="four" v-if="item.fields.結論與照片.length <= 4">
              <div v-for="(data, index) in item.fields.結論與照片" :key="index">
                <img :src="data.url" alt="" />
              </div>
            </div>
            <div class="more" v-else>
              <div
                v-for="(data, index) in item.fields.結論與照片"
                :key="data.id"
              ...
              >
                <div v-if="index === 3">
                  ...
                  >
                    <p>+{{ item.fields.結論與照片.length - 4 }}</p>
                  </button>
                </div>
                <div v-else-if="index === 4" :class="{ show: isShow }">
                  ...
                  <button
                    ...
                    class="count_more"
                    @click="isShowChange(data.id)"
                    v-if="item.fields.結論與照片.length > 6"
                  >
                    <p>+{{ item.fields.結論與照片.length - 5 }}</p>
                  </button>
                </div>
                <div
                  class="hide"
